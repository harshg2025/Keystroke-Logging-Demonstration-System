...
> This project is built strictly for cybersecurity awareness, academic learning, and ethical demonstration.
> Never use keystroke logging to monitor others without explicit consent. Doing so is **illegal** in most jurisdictions.
...
- **Keystroke data** is encrypted with [Fernet](https://cryptography.io/en/latest/fernet/) (AES-128-CBC + HMAC-SHA256)
...
1. **Consent is mandatory** — Only use on systems you own or have explicit written permission to monitor
2. **No background operation** — Logging only starts when the user clicks "Start Logging"
3. **No data exfiltration** — All data stays local; no network transmission beyond localhost
4. **No stealth features** — The UI is always visible; no system tray hiding
5. **Educational context only** — Intended for classroom, lab, and security awareness training
